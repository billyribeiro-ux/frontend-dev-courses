...
`Tween` accepts `duration`, `easing`, and an `interpolate` function for complex types:
...
You can also use the static `Tween.of` method to create a tween that automatically tracks a reactive expression:
...
// tweened.current smoothly follows `target` whenever it changes
```
...
Like `Tween`, `Spring` also has a static `Spring.of` method for automatically tracking reactive values:
...
// springCoords.current follows target with spring physics
```
...
If you need a manual approach (for example, in environments where `svelte/motion` is not available), you can use `MediaQuery` from `svelte/reactivity`:
...
- `new Tween(value, options)` from `svelte/motion` interpolates between values over time — great for progress bars, counters, and gauges
- `new Spring(value, options)` from `svelte/motion` provides physics-based motion with `stiffness` and `damping` — great for drag interactions and playful UIs
...
- The `interpolate` option on `Tween` lets you animate complex values like colors or objects
